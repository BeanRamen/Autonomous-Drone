...
class OffboardControl(Node):
    def __init__(self) -> None:
        ...
        self.obstacle_distance_threshold = 20.0 
        self.vehicle_position = VehicleLocalPosition()
        self.vehicle_status = VehicleStatus()
        self.current_waypoint_index = 0
        self.waypoints = [[0.0, 0.0, -10.0], [50.0, 0.0, -10.0], [500.0, 300.0, -10.0], [0.0, 300.0, -10.0], [0.0, 0.0, -10.0]]
    ...
    def lidar_callback(self, msg):
        ...
        if not any(r > 2 and not math.isinf(r) for r in msg.ranges):
            self.get_logger().info("Nu există obstacole detectate.")
            self.obstacle_detected = False
            return
        
        min_distance = min(r for r in msg.ranges if not math.isinf(r) and r > 2)
        ...
        obsright = obsleft = obscenter = ''

        if min(right) == min_distance:
            obsright = "dreapta"

        if min(center) == min_distance:
            obscenter = "mijloc"
            
        if min(left) == min_distance:
            obsleft = "stanga"

        self.get_logger().info(f"OBSTACOL detectat la {min_distance:.2f}m -> {obsright} - {obscenter} - {obsleft} ")
        
        self.obstacle_detected = min_distance < self.obstacle_distance_threshold

        if self.obstacle_detected:
            #self.get_logger().info("OBSTACOL detectat {}".format(min_distance))
            self.avoid_obstacle(min_distance)
        #self.get_logger().info("ceva")


    def avoid_obstacle(self,min_distance):
        """Funcție pentru evitarea obstacolului"""
        x, y, z = self.waypoints[self.current_waypoint_index]
        avoidance_distance = max(15, min_distance / 2)  # Setăm o distanță de evitare

        # Definim noi setări pentru a evita obstacolul treptat
        new_x, new_y = 0, - 5.0
        #self.get_logger().info("AAAAA: {}".format(float(new_x)))
    ...
    def publish_position_setpoint(self, x: float, y: float, z: float):
        ...
        msg.velocity = [0.05, 0.05, 0.05]
        msg.timestamp = int(self.get_clock().now().nanoseconds / 1000)
        self.trajectory_setpoint_publisher.publish(msg)
        #self.get_logger().info(f"Moving slowly to: {[x, y, z]}")
        #self.get_logger().info(f"Moving slowly with velocity: {msg.velocity}")
